...
#define DEQ(mon, x) ((cdate[0] == mon[0] && cdate[1] == mon[1] && cdate[2] == mon[2]) ? x : 0)
  ...
  /* Test 1: mod q timing */
  nn r;
    long s_cycle = (long) read_csr(mcycle);
  ret = kiet_nn_get_random_mod(&r, &(curve_params.ec_gen_order));
//   ret = nn_mod_notrim(&r, &r, &qprime);
    long e_cycle = (long) read_csr(mcycle);
    kprintf("==> Random mod API cycle: %d\n", e_cycle - s_cycle);
  if(ret != 0){
    kprintf("-> ERROR gen temporary number\n");
    goto end;
  }
  my_nn_print("Test random number", &r);
  kprintf("Random timing DONE\n\n");

 /* Test 2: Scalar point multiplication timing */
  prj_pt point;
    s_cycle = (long) read_csr(mcycle);
  ret = kiet_prj_pt_mul(&point, &r, &(curve_params.ec_gen));
    e_cycle = (long) read_csr(mcycle);
    kprintf("==> Scalar point mult API cycle: %d\n", e_cycle - s_cycle);
  if(ret != 0){
      kprintf("-> Server ERROR gen public key\n");
      goto end;
  }
  my_ec_point_print("Test random point: ", &point);
  kprintf("Scalar point multiplication timing DONE\n\n");

  /* Test 3: Hash timing */
  unsigned char demo[CERT_SIZE];
  for(int i = 0; i < CERT_SIZE; i++){
    demo[i] = "a";
  }
  unsigned char hashed_str[HASH_SIZE];

    s_cycle = (long) read_csr(mcycle);
  hash_implicit_cert(demo, hashed_str);
    e_cycle = (long) read_csr(mcycle);
  kprintf("==> Hash API cycle: %d\n", e_cycle - s_cycle);
  kprintf("Hashed demo: \n");
  my_bio_dump(hashed_str, HASH_SIZE);
  kprintf("Hash timing DONE\n\n");

  /* Test 4: big number multiplication and addition   */
  nn a;
  nn b;
  nn sum;
  nn mul;
  ret = kiet_nn_get_random_mod(&a, &(curve_params.ec_gen_order));
  ret = kiet_nn_get_random_mod(&b, &(curve_params.ec_gen_order));
    s_cycle = (long) read_csr(mcycle);
  ret = nn_mul(&sum, &a, &b);
    e_cycle = (long) read_csr(mcycle);
  kprintf("==> Big number multiplication cycle: %d\n", e_cycle - s_cycle);
  s_cycle = (long) read_csr(mcycle);
  ret = nn_add(&sum, &a, &b);
    e_cycle = (long) read_csr(mcycle);
  kprintf("==> Big number addition cycle: %d\n", e_cycle - s_cycle);
  kprintf("Big number multiplication and addition timing DONE\n\n");


  /* Test 5: point addition */
  prj_pt point2;
  prj_pt pt_sum;
  ret = kiet_prj_pt_mul(&point2, &a, &(curve_params.ec_gen));

    s_cycle = (long) read_csr(mcycle);
  ret = kiet_prj_pt_add(&pt_sum, &point, &point2);
    e_cycle = (long) read_csr(mcycle);
  kprintf("==> Point addition API cycle: %d\n", e_cycle - s_cycle);

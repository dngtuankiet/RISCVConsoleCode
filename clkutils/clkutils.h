...
// to delay up to period_ns-1 (for example), you wouldn't delay
// at all. So this function delays AT LEAST delay_ns.
inline void clkutils_delay_ns(int delay_ns, int period_ns) {
  uint64_t now = clkutils_read_mtime();
  uint64_t then = now + delay_ns / period_ns + 1;
